...
from eval.videomme.inference.ddp import *
...
def eval_model(args):
    ...
    dataset = EvalDatasetVideoMME(args.question_dir, args.video_folder, image_processor, video_processor)
    ...
    answer_data = []
    for (idx, sample_set, video_frames, context_frames, slice_len, original_sample) in tqdm(dataloader):
        ...
        sample = sample_set
        for sub_idx, qs in enumerate(sample['Q']):
            qs = qs[0]
            ...
            try:
                ...
                original_sample['questions'][sub_idx]['response'] = outputs
            except Exception as e:
                trace = traceback.format_exc()
                print(f"Error processing video file '{sample['video_name'][0]}': {e}")
                print("Detailed traceback:")
                print(trace)
        result = {'video_id':original_sample['video_id'][0],
               'duration':original_sample['duration'][0],
               'domain':original_sample['domain'][0],
               'sub_category':original_sample['sub_category'][0],
               'videoID':original_sample['videoID'][0],
               'questions':original_sample['questions']}
        for item in result['questions']:
            item['question_id'] = item['question_id'][0]
            item['task_type'] = item['task_type'][0]
            item['question'] = item['question'][0]
            item['answer'] = item['answer'][0]
            item['options'] = [op[0] for op in item['options']]
