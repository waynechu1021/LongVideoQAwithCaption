...
from eval.tempcompass.inference.ddp import *
...
def eval_model(args):
    ...
    dataset = EvalDatasetTempCompass(args.question_dir, args.video_folder, image_processor,
                                 video_processor)
    ...
    for (idx, sample_set, video_frames, context_frames, slice_len) in tqdm(dataloader):
        ...
        video_name = sample_set['video_name'][0]
        assert video_name not in predictions
        predictions[video_name] = {}
        sample = sample_set

        for dim, questions in sample['data'].items():
            predictions[video_name][dim] = []
            for question in questions:
                qs = question['question'][0]
                try:
                    cur_prompt = qs
