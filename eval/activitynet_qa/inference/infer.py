...
from eval.activitynet_qa.inference.ddp import *
...
def eval_model(args):
    ...
    dataset = EvalDatasetActivityNetQA(args.question_dir, args.video_folder, image_processor,
                                 video_processor)
    ...
    stopping_criteria = StoppingCriteriaList([StoppingCriteriaSub(stops=[torch.tensor([1919]).to('cuda'),
                                                                         torch.tensor([29892]).to('cuda')])])
    answer_data = []
    for (idx, sample_set, video_frames, context_frames, slice_len) in tqdm(dataloader):
        ...
        try:
            ...
            results = {"question_id": sample['question_id'][0],
                       "answer": outputs,}
